...
where
    Reader: FnMut(&mut IO, u64, &mut [u8]) -> Result<usize, <File<'fs, IO, TP, OCC> as IoBase>::Error>,
{
    fn read(&mut self, buf: &mut [u8]) -> Result<usize, Self::Error> {
        trace!("File::read");
...
impl<'fs, IO: ReadWriteSeek, TP: TimeProvider, OCC, Reader, Writer> Write
    for ReadWriteProxy<'_, 'fs, IO, TP, OCC, Reader, Writer>
where
    Writer: FnMut(&mut IO, u64, &[u8]) -> Result<usize, <File<'fs, IO, TP, OCC> as IoBase>::Error>,
{
    fn write(&mut self, buf: &[u8]) -> Result<usize, Self::Error> {
        trace!("File::write");
        let file = &mut self.file;
